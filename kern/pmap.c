...
page_free(struct PageInfo *pp)
{
	...
	assert(pp && !pp->pp_link && !pp->pp_ref);
...
mmio_map_region(physaddr_t pa, size_t size)
{
	...
	base += size;
	assert(base <= MMIOLIM);
	boot_map_region(kern_pgdir, result, size, pa, PTE_W | PTE_PCD | PTE_PWT);
	return (void *)result;
}
...
user_mem_check(struct Env *env, const void *va, size_t len, int perm)
{
	...
	for (uintptr_t v = va_begin; v < va_end; v += PGSIZE) {
		pte_t *pte = pgdir_walk(env->env_pgdir, (const void *)v, false);
